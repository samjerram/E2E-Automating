...
## Git tag: `checkpoint/presentation-stable` (2026-04-13)

**Purpose:** Lock a **simple presentation** baseline: only **demo2** (Regression Pack) and **demo3** (Bulk Order) in the UI; formatting guide labels match those names; auth hints updated; Flask rejects other `mode` values.

**Includes (commits on `main` at tag time):**

- `app.py` — mode guard, Windows login browser sizing/focus, post-login frontend focus without extra tabs where possible.
- `templates/index.html` — two run modes + copy updates.
- `make_backup_zip.sh`, `start_app.command` — incidental mode/metadata in tree.
